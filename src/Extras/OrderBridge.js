...
const oBridge       = {
    ...
    add_order_to_store                  : function (objPayload){
        let item_id     = objPayload.id;
        let item_aid    = objPayload.aid;
        let item_cid    = objPayload.cid;
        ...
        if( this.array_key_exists(item_id,  this.product_store) ){
            this.product_store    = this.process_order(item_id, order_strip);
        }else{
            this.product_store[item_id]       = order_strip;
            this.store_session.set(this.namespace, this.baseKey, this.product_store);
        }

        return this.get_context_payload( item_id, qty, true);
    },
    ...
    delete_order_from_store             : function (objPayload){
        ...
        if(this.array_key_exists(item_id,  this.product_store) ){
            this.product_store    = this.process_order(item_id, order_strip, 0);
            if(!this.array_key_exists(item_id,  this.product_store)){
                return {message :  'All items successfully deleted.', slotData: '--'};
            }
        }

        return (a = this.get_context_payload(item_id, qty, false) )? a : null;
    },

    process_order                       : function (item_id, order_strip, add_or_remove){
        ...
        if(this.array_key_exists(item_id, this.order_context_obj)  ){
            //ARE WE ADDING OR REMOVING???  || ADD = 1, REMOVE = 0
            let orderObj    = this.order_context_obj[item_id];
            if(add_or_remove){
    ...
    get_context_payload                 : function (item_id, qty, add){
        ...
        if(undefined === this.product_store[item_id]) {
            return null;
        }
        ...
        if( Object.size(context_obj_ar) > 0){
            ...
            if( this.isSet(context_obj_ar[item_id])){
                return context_obj_ar[item_id];
            }
        }
        return null;
    },
